...
const button_randomization = jsPsych.randomization.sampleWithoutReplacement(["medicine_high", "medicine_low"], 1)[0]

const medicine = stim.filter(function(s){return s.pilule === "medicine"; }); // keep only medicine trials
const placebo = stim.filter(function(s){return s.pilule === "placebo"; });

const medicine_randomization = jsPsych.randomization.repeat(medicine, 6); // each is repeated 6 times, which gives 4*6 = 24
const placebo_randomization = jsPsych.randomization.repeat(placebo, 6);

// In conjunction, this gives MR = 18, MNR = 6 (p = ,75) and PR = 18, PNR = 6 (p = ,75)

const order_randomization = jsPsych.randomization.sampleWithoutReplacement(["medicine_first", "placebo_first"], 1)[0]
const stim_randomization = [];

for (let i = 0; i < 24; i++) { // 24 times med and pla (or pla and med), so 48 trials in total
...
const loop_pilule = {
  ...
  loop_function: function(){
    const response = jsPsych.data.get().last().values()[0].response;
    const correct_button = jsPsych.timelineVariable('correct_button')
...
const instruction_demographic_questionnary = {
  type: jsPsychHtmlButtonResponse,
  stimulus:
